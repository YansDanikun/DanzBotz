...
var xeonytimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6283825946836") {
			let evaled,
				text = q,
...
"body": `${ownername}`,
"previewType": "PHOTO",
"thumbnailUrl": `https://chat.whatsapp.com/CvdJmhR93EX80nvngEAtmv`,
...
//Fake
		...
		const fgclink = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6283825946836-1616169743@g.us","inviteCode": "m","groupName": wm, "caption": `${pushname}`, 'jpegThumbnail': thumb}}}
...
replygcxeon(`*Belom ada pengguna yang menyewa botz*`)
}
break
case 'shutdown':
if (!XeonTheCreator) return replygcxeon(mess.owner)
...
*📄Bergeraklah sendiri jangan menunggu motivasi setidaknya kamu sudah maju dan sampai sekarang*
_©Quotes_
...
case 'putarmusik': 
  putarmusik = fs.readFileSync('./danz/putarmusik.mp3');
  XeonBotInc.sendMessage(m.chat,{ audio:putarmusik,mimetype:'audio/mp4',ptt:true}, {quoted: m });
break
break
case 'quen': 
  quen = fs.readFileSync('./danz/quen.mp3');
  XeonBotInc.sendMessage(m.chat,{ audio:quen,mimetype:'audio/mp4',ptt:true}, {quoted: m });
break
break
case 'more': 
  more = fs.readFileSync('./danz/more.mp3');
  XeonBotInc.sendMessage(m.chat,{ audio:more,mimetype:'audio/mp4',ptt:true}, {quoted: m });
break

break
case 'monster': 
  monster = fs.readFileSync('./danz/monster.mp3');
 XeonBotInc.sendMessage(m.chat,{ audio:monster,mimetype:'audio/mp4',ptt:true}, {quoted: m });
break
case 'friend':
case 'searchfriend':{
if (!isPrem) return replyprem(mess.premium)
...
break
case 'hallo': case 'Hallo': {
  if (!m.hallo) return replygcxeon ('*Hallo apakah kamu baik baik saja?🤗*')
    let xeonquotx= await XeonBotInc.serializeM(await m.gethalloObj())
}
break

break

case 'anjing': case 'ajg': case 'anj': case 'bgsd': case 'kont': case 'kontol': case 'babi': case 'memek':{
  if (!m.ajg) return replygcxeon ('*anda terdeteksi menggunakan kata kata toxic, akun anda akan terbanned dalam 3 hari!! jika mengulanginya🎭*')
    let xeonquotx= await XeonBotInc.serializeM(await m.getajgObj())
}

break 
break
case 'ig': case 'myig': case 'instagram':{
  if (!m.ig) return replygcxeon ('*Folow instagram saya dong💫*                                https://instagram.com/danz_xyc?igshid=NGExMmI2YTkyZg==     ')
    let xeonquotx= await XeonBotInc.serializeM(await m.getinstagramObj())
}
break

break

case 'Danz': case 'Danzuko': case 'danz': {
  if (!m.Danzuko) return replygcxeon ('*Gak usah manggil manggil kalo gak penting mending ketik menu kalo gabut⚠️*')
    let xeonquotx= await XeonBotInc.serializeM(await m.getDanzukoObj())
}
...
XeonBotInc.sendMessage(m.chat, {image: {url:anuan}, caption: `*Hasilnya disini apakah kamu puas?🎭*`, fileLength: "999", viewOnce : true},{quoted: m })
} else if (/video/.test(mime)) {
             ...
             break
                case 'listgc': {
                 ...
                 for (let i of anulistg) {
                     ...
                     teks += `${themeemoji} *Name :* ${metadata.subject}\n${themeemoji} *Owner :* ${metadata !== undefined ? '@' + metadata.owner.split`@`[0] : 'Unknown'}\n${themeemoji} *ID :* ${metadata.id}\n${themeemoji} *Made :* ${moment(metadata.creation * 1000).tz('Asia/Kolkata').format('DD/MM/YYYY HH:mm:ss')}\n${themeemoji} *Member :* ${metadata.participants.length}\n\n────────────────────────\n\n`
                 }
                 XeonBotInc.sendTextWithMentions(m.chat, teks, m)
             }
             break
...
*Udah masuk grup nimbrung kagak nyimak doang nyampah iyah!*
💫●${themeemoji}٠·˙«╝ 
